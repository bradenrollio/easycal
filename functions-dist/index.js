...
async function onRequest6(context) {
  ...
  try {
    ...
    const tokenData = await getLocationToken2(locationId, env);
    if (!tokenData) {
      return new Response(JSON.stringify({
        error: "No access token found for location"
      }), {
...
async function getLocationToken2(locationId, env) {
  try {
    const result = await env.DB.prepare(`
      SELECT access_token, refresh_token, expires_at 
      FROM tokens 
...
async function onRequest7(context) {
  ...
  try {
    ...
    const tokenData = await getLocationToken3(locationId, env);
    if (!tokenData) {
      return new Response(JSON.stringify({
        timeZone: "America/New_York"
      }), {
...
async function getLocationToken3(locationId, env) {
  try {
    const result = await env.DB.prepare(`
      SELECT access_token, refresh_token, expires_at 
      FROM tokens 
...
// ../.wrangler/tmp/pages-rtEMru/functionsRoutes-0.17178893795099204.mjs
var routes = [
  {
...
// ../../../.nvm/versions/node/v22.16.0/lib/node_modules/wrangler/node_modules/path-to-regexp/dist.es2015/index.js
function lexer(str) {
  var tokens = [];
  var i = 0;
  while (i < str.length) {
...
// ../../../.nvm/versions/node/v22.16.0/lib/node_modules/wrangler/templates/pages-template-worker.ts
var escapeRegex = /[.+?^${}()|[\]\\]/g;
