...
// ../.wrangler/tmp/bundle-5DwOyO/checked-fetch.js
...
async function handleListCalendars(accessToken, locationId2, corsHeaders) {
  try {
    const actualLocationId = locationId2.startsWith("agency_") ? "HgTZdA5INm0uiGh9KvHC" : locationId2;
...
async function handleDeleteCalendars(request, accessToken, locationId2, corsHeaders) {
  try {
    ...
    for (const calendarId of calendarIds) {
      try {
        const response = await fetch(`https://services.leadconnectorhq.com/calendars/${calendarId}?locationId=${locationId2}`, {
          method: "DELETE",
          headers: {
            "Authorization": `Bearer ${accessToken}`,
            "Version": "2021-07-28",
  ...
  try {
    ...
    for (let i = 0; i < csvRows.length; i++) {
      ...
      try {
        ...
        const successResult = {
          success: !result.isPending,
          // Only count as success if not pending
          ...
          warnings: validationErrors.filter((e) => e.severity === "warning").map((e) => e.message),
          data: result.data
          // Include the payload data for pending calendars
        };
    ...
    const failedResults = results.filter((r) => !r.success && !r.isPending);
    return new Response(JSON.stringify({
      success: true,
      jobId,
      results,
      summary: {
        ...
        updated: successfulResults.filter((r) => r.isUpdate).length,
        created: successfulResults.filter((r) => !r.isUpdate).length,
        message: pendingResults.length > 0 ? `Note: GoHighLevel API does not currently support calendar creation. ${pendingResults.length} calendar(s) prepared for manual creation.` : null,
        apiNote: pendingResults.length > 0 ? "Calendar data has been validated and prepared. You can create these calendars manually in GoHighLevel or wait for API support." : null
      }
    }), {
      status: 200,
      headers: { "Content-Type": "application/json", ...corsHeaders }
...
function validateCSVRow(row, rowIndex) {
  ...
  if (!hasScheduleBlocks && hasDayAndTime) {
    const dayAbbr = row.day_of_week.substring(0, 3);
...
function buildCalendarPayload(row, brandConfig, defaults, locationId2, groupId) {
  ...
  return {
    ...
    slug,
    widgetType: "default",
    // Remove fields that might be causing 404
    // calendarType: 1,
    ...
    widgetColor: branding.primaryColor,
    // Use simpler time settings
    slotDuration: parseInt(row.slot_interval_minutes) || 30,
    slotBuffer: 0,
    preBuffer: 0,
    appoinmentPerSlot: 1,
    appoinmentPerDay: parseInt(row.max_bookings_per_day) || 10,
...
async function createOrUpdateCalendar(payload, accessToken) {
  try {
    console.log("Processing calendar:", payload.name, "with slug:", payload.slug);
    ...
    if (existingCalendar) {
      ...
      return {
        ...
        isUpdate: true,
        message: "Calendar already exists (updates not supported via API)"
      };
    } else {
      console.warn("Calendar creation is not supported via GoHighLevel API v2");
      console.log("Calendar data that would be created:", JSON.stringify(payload, null, 2));
      return {
        id: `pending_${Date.now()}_${payload.slug}`,
        isUpdate: false,
        isPending: true,
        message: "Calendar creation pending (not yet supported by GHL API)",
        data: payload
      };
    }
  } catch (error) {
...
async function onRequest10(context) {
  ...
  try {
    ...
    if (!locationId2 && (userType === "Company" || isBulkInstallation)) {
      ...
      const encryptedTokens2 = await encryptTokens({
        accessToken: tokenData.access_token,
        refreshToken: tokenData.refresh_token
      }, env.ENCRYPTION_KEY);
...
__name(encryptTokens2, "encryptTokens");
...
// ../.wrangler/tmp/pages-UpVmg7/functionsRoutes-0.30050475990650705.mjs
var routes = [
  {
...
// ../.wrangler/tmp/bundle-5DwOyO/middleware-insertion-facade.js
...
// ../.wrangler/tmp/bundle-5DwOyO/middleware-loader.entry.ts
...
//# sourceMappingURL=functionsWorker-0.1699063320002271.mjs.map
